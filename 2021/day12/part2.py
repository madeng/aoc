...
test_expected_results = [36, 103, 3509]
...
def build_paths(node_map: dict, node_name: str, current_path: [], small_consumed=False):
    if re.match(r"[a-z]", node_name) and node_name in current_path:
        if small_consumed is True:
            return None
        else:
            small_consumed = True
    ...
    if node_name == 'end':
        ...
        for n in node_map[node_name]:
            new_paths = build_paths(node_map, n, current_path.copy(), small_consumed)
